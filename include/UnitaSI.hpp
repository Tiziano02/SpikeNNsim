...
/*
 * UnitaSI — sistema di unità fisiche SI per la simulazione.
 *
 * Tutte le costanti sono fattori di scala rispetto alle unità SI base.
 * Moltiplicare un valore numerico per la costante corrispondente lo esprime
 * nell'unità SI base corretta.
 *
 * Esempio:
 *   double V = -65.0 * mV;  // -0.065 V in unità SI
 *   double t =   5.0 * ms;  //  0.005 s in unità SI
 *
 * In questo modo l'integrazione numerica lavora sempre con valori coerenti
 * senza richiedere conversioni manuali nelle equazioni differenziali.
 *
 * Le unità derivate (Hz, Volt, Ohm, ...) sono definite a partire dalle
 * unità fondamentali per garantire la coerenza dimensionale per costruzione.
 * Le unità non direttamente usate nella simulazione (v, a, rho, Pa) sono
 * mantenute perché necessarie nella catena di definizione delle unità derivate.
 */
...
// unità derivate — definite dalla catena SI
...
constexpr double S    = A / Volt;          // conduttanza       [S]  -- attenzione: nome breve, può confondere
...
// alias comodi per la simulazione neurale
